//A small server daemon that accepts communication from it's companion client program, otp_enc
// The purpose of this program is to accept a plaintext file and an OTP key, and to encrypt
// the plaintext file, using the key, via the OTP process. Encrypted data is then sent back
// to the client.
...
//Encrypts the plaintext string using the key string. Returns the encrypted string
char* encrypt(char* plaintext, char* key)
{
	...
	encString = calloc(strlen(plaintext), sizeof(char));
	assert(encString != NULL);

	for(i = 0;i < strlen(plaintext);i++)
	{
		p = convertChar(plaintext[i]);
		k = convertChar(key[i]);
		temp = p + k;
		temp = temp % 27;
		tempChar = convertInt(temp);
		strcat(encString, tempChar);
	}
	...
	while(1)
	{
		...
		switch(spawnPid)
		{
			case 0:
				...
				//Kill connection if connecting program not 'otp_enc'
				...
				//Receive the number of chars incoming from plaintext
				...
				//Receive plaintext string
				//Allocate memory for the buffer and plaintext
				...
				//Send encodedtext back to client
				charsWritten = send(estConnFD, enctext, strlen(enctext), 0);
				if(charsWritten < 0)
				{
					fprintf(stderr, "Error writing to socket");
				}
